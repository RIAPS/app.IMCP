...
def build_connected_group(my_group_id: int, topology: dict, status_of_relays: dict):
    ...
    for group_id, connection_status in enumerate(connection_matrix[my_group_id]):
        electrically_independent_group_members = topology[
            "electrically_independent_groups"
        ][group_id]
        electrically_independent_group_name = topology[
            "electrically_independent_group_names"
        ][str(electrically_independent_group_members)]
...
class GROUP_MANAGER(Component):
    ...
    def __init__(self, config, topology_config):
        ...
        self.MAX_GROUP_PUBLISH_DELAY_SECONDS = modbus_device_config[
            "MAX_GROUP_PUBLISH_DELAY_SECONDS"
        ]
        ...
        self.electrically_independent_groups = group_config[
            "electrically_independent_groups"
        ]
    ...
    def on_relay_sub(self):
        ...
        if (
            relay_status_has_changed
            or time_since_last_broadcast > self.MAX_GROUP_PUBLISH_DELAY_SECONDS
        ):
    ...
    def group_management(self):
        group, group_name = build_connected_group(
            my_group_id=self.my_group_id,
    ...
    def future_group_management(
        self, group, group_name, requested_relay, requested_action
    ):
        ...
        future_group, future_group_name = build_connected_group(
            my_group_id=self.my_group_id,
            topology=self.group_config,
    ...
    def on_operator_sub(self):
        ...
        self.future_group, self.future_group_name = self.future_group_management(
            self.group, self.group_name, self.requestedRelay, self.requestedAction
        )  # sets self.futureGroup

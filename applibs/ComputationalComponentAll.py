...
class ComputationalComponent(Component):
    def __init__(self, config, Ts, topology_config):
        ...
        self.DIRECTLY_CONNECTED_RELAYS = topology_cfg["DIRECTLY_CONNECTED_RELAYS"][
            self.pccRelayID
        ]
        ...
        self.initialReactivePowerCommand = modbus_device_config[
            "Initial_reactive_power"
        ]
        self.initialPowerCommand = [
            self.initialActivePowerCommand,
        ...
        self.regulationPower = [
            const.default_regulation_active_power,
            const.default_regulation_reactive_power,
        ]

        """ APP and CONSENSUS CLASS """
        # distributed voltage estimation app
        self.estimateApp = apy.VoltageEstimation(
            "dynamic", self.Ts, debugMode, logger=self.logger
        )
        self.estimateApp.consensus.setGains(const.consensus_gain)

        # Resycnhronizaztion to the Grid
        self.resyncApp = apy.ResynchronizationControl(
            "aver",
            [self.nominalFrequency, self.nominalVoltage],
            self.Ts,
            debugMode,
            logger=self.logger,
        )
        self.resyncApp.consensus.setGains(
        ...
        self.reconfigRelayCloseApp = apy.RelayCloseControl(
            "aver",
            [self.nominalFrequency, self.nominalVoltage],
            self.Ts,
            debugMode,
        ...
        self.reconfigRelayCloseApp.consensus.setGains(
            (const.active_pwr_ctrl_gain, const.reactive_pwr_ctrl_gain),
            (const.freq_ctrl_gain, const.voltage_ctrl_gain_relay),
        )

        # Reconfiguration app to open relay
        self.reconfigRelayOpenApp = apy.RelayOpenControl(
            "aver",
            [self.nominalFrequency, self.nominalVoltage],
            self.initialPowerCommand,
            None,
        ...
        self.economicApp = apy.EconomicDispatch(
            "aver", self.costFunction, self.Ts, debugMode, logger=self.logger
        )
        self.economicApp.consensus.setGains(
            (const.alpha_x_gain, const.edp_reactive_pwr_ctrl_gain),
        ...
        log_json(
            self.logger,
            "info",
            f"{str(self.pid)} - starting",
            event="COMPONENT_STARTING",
        )
    ...
    def on_state_sub(self):
        ...
        msg = from_bytes(imcp_capnp.StateMsg, msg_bytes).to_dict()
        ...
        if self.group_members != msg.get("group"):
            log_json(
                self.logger,
                "info",
                f"Group members changed from {self.group_members} to {msg.get('group')}",
                event={
                    "name": "GROUP_MEMBERS_CHANGED",
    ...
    def on_relay_sub(self):
        ...
        if not self.appInitReady:
            self.logger.info(
                f"self.relay_sub.connected(): {self.relay_sub.connected()}"
            )
            if self.pccRelay_closed is None:
                (
                    self.logger.warn(
                        f"{helper.Red} ComputationalComponentAll.py | on_relay_sub | state of PCC relay is unknown. {helper.RESET}"
                    )
                    if debugMode
                    else None
                )
            ...
            self.logger.info(
                f"ComputationalComponentAll.py - on_relay_sub | "
    ...
    def on_consensus_clock(self):
        ...
        msg.params = [
            ...
            "VREF",
            "WREF",
        ]  # frequency, voltageMag, activePower, reactivePower
        ...
        if debugMode:
            self.logger.info(
                f"{helper.qryBlack} | ComputationalComponent.py - on_consensus_clock \n"
        ...
        try:
            ...
            self.logger.info(
                f"{helper.White}" f"on_device_port queue full: {e}" f"{helper.RESET}"
            )
    ...
    def on_device_qry_port(self):
        ...
        frequency, voltageMag, activePower, reactivePower, vref, wref = [
            value[0] for value in msg.values
        ]
        ...
        incremental_cost = self.economicApp.getIncrementalCost(
            [activePower, reactivePower]
        )
        estimated_voltage_share = self.estimateApp.get_estimated_voltage_share(
            voltagePU
        )
        ...
        # do app if in one of the ACTIVE STATES
            ...
            if False:
                ...
                group_consensus_msg.estimatedVoltageShare = (
                    self.estimateApp.get_estimated_voltage_share()
                )
                ...
                if self.currentState == "ACTIVE_GRID-TIED":
                    ...
                    self.economicApp.run(
                        my_data_values,
                        self.consensus_data,
                        self.relayMessages[self.pccRelayID],
                        self.regulationPower,
                        self.pinningDG,
                        power_command,
                    )
                    group_consensus_msg.incrementalCost = (
                        self.economicApp.getIncrementalCost()
                    )
                    ...
                    if self.stateBreaker in self.relayMessages:
                        ...
                        self.reconfigRelayCloseApp.run(
                            my_data_values,
                            self.consensus_data,
                        ...
                        self.activeFrequencyAndVoltageShift = (
                            self.reconfigRelayCloseApp.frequencyAndVoltageOffset
                        )
                    ...
                    self.oldState = self.currentState
                elif self.currentState in [
                    "ACTIVE_PREPARE-DISCONNECT",
                    "ACTIVE_PREPARE-ISLAND",
                ]:
                    if self.stateBreaker in self.relayMessages:
                        if self.oldState not in [
                            ...
                            active_frequency_and_voltage_shift = [
                                (wref - 1) * self.nominalFrequency,
                        ...
                        self.activeFrequencyAndVoltageShift = (
                            self.reconfigRelayOpenApp.frequencyAndVoltageOffset
                        )
                    ...
                    self.activeFrequencyAndVoltageShift = (
                        self.resyncApp.frequencyAndVoltageOffset
                    )
                    ...
                    if True:
                        if (
                            self.oldState == "ACTIVE_PREPARE-DISCONNECT"
                            and self.reconfigRelayOpenApp.usePowerCommand
                        ):
                            self.activeFrequencyAndVoltageShift = [0.0, 0.0]
                        self.secondApp.run(
                            my_data_values,
                        ...
                        self.activeFrequencyAndVoltageShift = (
                            self.secondApp.frequencyAndVoltageOffset
                        )
                    ...
                    self.logger.warn(
                        f"No app is selected, this should NOT happen (or should set to default commands?)"
                    )
        ...
        try:
            ...
            self.logger.info(
                f"{helper.Red}" f"on_device_port queue full: {e}" f"{helper.RESET}"
            )
        ...
        try:
            ...
            self.logger.info(
                f"{helper.White}" f"consensus_pub queue full: {e}" f"{helper.RESET}"
            )
    ...
    def on_consensus_sub(self):
        ...
        msg = from_bytes(imcp_capnp.DgGeneralMsg, msg_bytes)

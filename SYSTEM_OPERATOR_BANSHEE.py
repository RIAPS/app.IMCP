...
class SYSTEM_OPERATOR(Component):
    ...
    def on_device_port(self):
        ...
        (
            self.startStop_opal,
            self.gridBreaker_opal,
            self.secondaryCtrl_opal,
            self.secondaryAngleCtrl_opal,
            self.regulationSignal,
            self.regulationSignal2,
        ) = msg.values
    ...
    def on_consensus_sub(self):
        ...
        capnp_msg = from_bytes(imcp_capnp.DgGeneralMsg, msg_bytes)
        self.logger.debug("System operator: on_consensus_sub" f"msg: {capnp_msg}")
    ...
    def on_relay_sub(self):
        ...
        capnp_msg = from_bytes(imcp_capnp.RelayMsg, msg_bytes)
        self.logger.debug("System operator: on_relay_sub" f"msg: {capnp_msg}")
    ...
    def ReconfigurationManager(self):
        ...
        if self.currentTestState == "RESET":
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F1PCC"]["status"] == "disconnected"
                and self.relayStatus["F2PCC"]["status"] == "disconnected"
                and self.relayStatus["F3PCC"]["status"] == "disconnected"
            ):
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F1108"]["status"] == "connected"
            ):
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F2217"]["status"] == "connected"
            ):
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F2217"]["status"] == "disconnected"
            ):
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F1108"]["status"] == "disconnected"
            ):
            ...
            if (
                self.testCounter20second >= 100
                and self.relayStatus["F1PCC"]["status"] == "connected"
                and self.relayStatus["F2PCC"]["status"] == "connected"
                and self.relayStatus["F3PCC"]["status"] == "connected"
            ):
        ...
        if True:
            if self.display_counter_Q >= msgcounterLimit:
                self.display_counter_Q = 0
                self.logger.info(
                    f"{helper.Yellow} test state {self.currentTestState} {helper.reset}"
                )

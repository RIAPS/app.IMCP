...
class FSM(IMCP_FSM):
    ...
    def convert_relay_msg_to_trigger(self, relay_id, connected, unplanned):
        ...
        if self.debugMode:
            log_json(
                self.logger,
                "info",
                "convert_relay_msg_to_trigger conditions",
                event={
                    "CONDITIONS": {
                        "relay_id": relay_id,
                        "connected": connected,
        ...
        if unplanned:
            if relay_id == self.pcc_relay_id:
                return (
                    ("unplannedIsland", False)
                    if not connected
                    else ("unplannedResync", False)
                )
            elif relay_id != self.pcc_relay_id:
                return (
                    ("unplannedDisconnect", False)
                    if not connected
                    else ("unplannedConnect", False)
                )
    ...
    def convert_requested_action_to_trigger(self, msg):
        ...
        if commandStartStop == 0 and self.model.state != "SHUTDOWN":
            ...
            return (
                None,
                False,
            )  # These are the predicates for the LOCAL-CONTROL state, so don't do anything.

        # second evaluate the local/active control command
        if (
            commandEnableActiveControl == 0 and self.model.state[0:7] == "ACTIVE_"
        ):  # only disable it when it is active
        ...
        # Feeder Operations
            ...
            # see https://github.com/RIAPS/app.MgManage/blob/new-fsm-integration/FSM.py#L196
                ...
                log_json(
                    ...
                    event={
                        "UNSUPPORTED REQUEST": {
                            "commandStartStop": commandStartStop,
                            "model_state": self.model.state,
    ...
    def handleActionVoteRequest(self, group, rfcId, when):
        ...
        self.logger.info(
            f"handleActionVoteRequest[{str(rfcId)}] = "
    ...
    def handleVoteResult(self, group, rfcId, vote):
        ...
        self.logger.info(
            f"vote_id: {rfcId} | handleVoteResult = {str(vote)} "
        ...
        if not vote_msg:
            log_json(
                self.logger,
                "warn",
                "We missed the vote request for {vote}, but got the result",
        ...
        if vote == "timeout":
            self.logger.info(
                f"{str(rfcId)} Poll expired before {vote_msg['kind']} was reached"
            )
            return
        ...
        self.model.trigger(
            vote_msg["trigger"],
            coordinated="True",
            info=f"Vote: {vote}, " f"kind: {vote_msg['kind']}, " f"rfcID: {str(rfcId)}",
        )
        ...
        self.logger.info(
            f"{str(rfcId)} CONSENSUS LATENCY: {now - vote_msg['vote_start']}"
        )
        self.logger.info(
            f"{str(rfcId)} relay and action: {self.relayUnderControl}:{self.actionUnderControl}"
        )
